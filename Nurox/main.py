...
class DebateResponse(BaseModel):
    ...
    authority: str
    confidence: str
    usage: dict   # NEW — returns usage stats to frontend
...
Use emojis. Use **bold** for key concepts.
Use structured sections. Avoid text walls.
"""

PROMPT_AUDITOR = """
Provide a decisive final professional conclusion.
Use emojis. Use **bold**. Keep it clean and structured.
"""
...
async def debate(
    ...
    db: Session = Depends(get_db)          # ← Proper session injection
):
    # ── USAGE CHECK (runs before any LLM call) ──
    ...
    # ── SAVE TO DB (using injected session, not manual open) ──
...
def get_history(
    ...
    ).order_by(DebateHistory.created_at.desc()).all()
    return records

# ============================
# USAGE STATS ENDPOINT (NEW)
# ============================
...
def get_usage(
    ...
    if not tracking:
        return {"plan": current_user.plan, "debates_today": 0, "debates_this_month": 0}
